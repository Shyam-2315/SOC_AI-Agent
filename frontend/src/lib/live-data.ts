...
export function alertFromRealtimeEvent(event: RealtimeEvent): AlertRecord {
  ...
  return {
    ...
    title: textOf(payload.title, textOf(payload.message, textOf(payload.event_type, "Alert"))),
    ...
    hostname: textOf(payload.hostname),
    host: textOf(payload.host),
    username: textOf(payload.username),
    domain: textOf(payload.domain),
    source_ip: textOf(payload.source_ip),
    logon_type: textOf(payload.logon_type),
    failure_reason: textOf(payload.failure_reason),
    failed_login_count:
      typeof payload.failed_login_count === "number" ? payload.failed_login_count : undefined,
