...
def load_config() -> dict[str, Any]:
    if not CONFIG_FILE.exists():
        raise RuntimeError("Missing config.json. Run: copy config.example.json config.json")
...
def to_soc_log(event: dict[str, Any]) -> dict[str, Any]:
    fields = event["fields"]
...
class WindowsEventCollector:
    ...
    def poll_once(self) -> None:
        ...
        if self.send_logs(logs):
            ...
            logging.warning("State not advanced because backend send failed")
...
def main() -> int:
    ...
    try:
        config = load_config()
        WindowsEventCollector(config).run()
        return 0
